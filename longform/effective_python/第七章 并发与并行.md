...
​		然后，为队列定义一个迭代器，用于查找这个特殊对象，并在找到它时停止迭代。这个 `__iter__` 方法也会在适当的时候调用task_done，来跟踪队列上的工作进度
...
​		不过，情况并不总是如此。目前还没有next和iter内置函数的异步版本(参见第31条);必须在 `__anext__` 和 `__aiter__` 方法上直接使用await。这里也没有异步版本的yield from(参见第33条)，这使得合成生成器变得更复杂。但是考虑到Python中添加异步功能的速度如此之快，这些特性的可用只是时间问题。
...
​		用tail_file包装tail_async之后，下一让asyncio的事件循环保持畅通，以便进一步提升程序的响应能力　步是将run_threads函数转换为协程。这与上面的自顶向下方法的第4步是相同的工作，所以在这一点上，样式是收敛的。
...
​		前面一条中(第63条)中，展示了如何以增量方式将项目迁移至asyncio。得到的协程正确地跟踪输入文件，并将它们合并为单个输出:
...
​		然而，，它仍然有一个大问题:对输出文件句柄的open、close和write调用发生在主事件循环中。这些操作都需要对程序的操作系统进行系统调用，这可能会在相当长的时间内阻塞事件循环，并阻止其他协程取得进展。这可能会损害整体响应能力并增加延迟，特别是对于像高并发服务器这样的程序。

​		可以通过向asyncio.run函数传递debug=True参数来检测何时发生此问题。在这里，将展示如何识别可能在缓慢的系统调用中阻塞的坏协程的文件和行
...
>>>
Executing <Task finished name='Task-1' coro=<slow_coroutine() done, defined at E:/pycharm_pro/EffectivePythonCode/item40/test.py:76> result=None created at D:\python38\lib\asyncio\base_events.py:591> took 0.500 seconds
```

​		如果想要尽可能响应性最强的程序，需要最小化事件循环中可能产生的系统调用。在这种情况下，可以创建一个新的Thread子类(参见第53条)，它使用自己的事件循环封装所有需要写入输出文件的内容
...
​		其他线程中的协程可以直接调用并等待这个类的write方法，因为它只是围绕real_write方法的线程安全包装器，而real_write方法实际执行I/O。这就消除了对Lock的需要(参见第54条)：
...
​		其他协程可以用线程安全的方式告诉工作线程何时停止，使用类似的样板文件:
...
​		还可以定义 `__aenter__` 和 `__aexit__` 方法，以允许在语句中使用该类(参见第66条)。这确保工作线程在正确的时间启动和停止，而不会减慢主事件循环线程:
...
​		有了这个新的WriteThread类，可以将run_tasks重构成一个完全异步的版本，这个版本易于阅读，并且可以完全避免在主事件循环线程中运行缓慢的系统调用:
...
* 在协程中进行系统调用——包括阻塞I/O和启动线程——可能会降低程序响应性并增加延迟的感觉
* 将debug=True参数传递给asyncio.run，以便检测某些协程何时阻止事件循环快速响应

## 第64条 考虑用concurrent.futures实现真正的并行计算

​		在编写Python程序时，可能会遇到瓶颈。即使在优化代码之后(参见第70条)，程序依旧可能很慢。在具有越来越多的CPU内核的现在计算机上，可以合理地假设并行性是一种解决方案。如果可以将代码的计算拆分成多个独立的工作快，这些 工作代码块可以在多个CPU核心上同时运行，那会怎么样呢？

​		不幸的是，Python的全局解释器锁(GIL)阻止了线程中真正的并行性(参见第53条)，因此这样做行不通。另一个常见的建议是使用C语言将最关键的性能代码重写为扩展模块。C语言更加纯粹，运行速度比Python快，在某些情况下消除了对并行性的需要。C扩展还可以启动独立于并行运行的Python解释器的本地线程，并利用多个CPU内核，而不关心GIL。Python用于C扩展的API有很好的文档说明，是逃生舱口的好选择。还有值得一试的工具，如SWIG (https://github.com/swig/swig)和CLIF (https://github.com/google/clif)，以帮助扩展开发。

​		但是用C重写代码的成本很高。在Python中简短易懂的代码在C中可能会变得很冗长和复杂。这样的移植需要大量的测试，以确保功能与最初的Python代码相同，并且没有引入任何bug。有事这样做是值得的，这解释了Python社区中C扩展模块的大型生态系统，他们加速了文本解析、图像合成和矩阵数学等工作。甚至还有一些开源工具，如CPython和Numba，他们可以轻松地过度到C语言。

​		问题是，在大多数情况下，将程序的一部分移动到C语言中是不够的。优化后的Python程序通常没有一个主要的慢源;相反，通常有许多重要的贡献者。为了获得C的纯粹和线程的好处，需要移植程序的大部分，这大大增加了测试需求和风险。一定有更好的方法来保留Python中的设计，以解决复杂的计算问题。

​		内置模块multiprocessing，可以通过concurrent.futures内置模块很容易的访问，可能会满足需求(参见第59条)。它能够使Python运行额外的解释器作为子进程来并行地利用多个CPU内核。这些子进程与主解释器是分开的，因此它们的全局解释器锁也是分开的。每个子节点可以充分利用一个CPU核心。每个子进程都有一个到主进程的链接，在主进程中它接收执行计算的指令并返回结果。

​		例如，假设想用用Python做一些计算密集型的工作，并利用多个CPU内核。这里将使用寻找两个数的最大公约数的实现作为计算强度更大的算法的代理(比如用Navier-Stokes方程模拟流体动力学)
...
​		串行运行这个函数需要线性增加的时间，因为没有并行性:

run_serial.py

```python
# run_serial.py
from example_code.item_64.parallel.my_module import gcd
...
​		在多个Python线程上运行此代码不会提高速度，因为GIL阻止Python并行使用多个CPU内核。在这里梦之星与上面相同的计算，但是使用并发。带有ThreadPoolExecutor类和两个工作线程的futures模块(用于匹配计算机上的CPU内核数):
...
​		由于启动线程池和与线程池通信的开销，这一次甚至更慢。

​		现在是令人惊讶的部分:更改一行代码会导致一些神奇的事情发生。如果用concurrent.futures模块的ProcessPoolExecutor替换ThreadPoolExecutor，一切加速:
...
​		在双核机器上运行，这是非常快的!这怎么可能呢?下面是ProcessPoolExecutor类实际做的事情(通过multiprocessing模块提供的低级结构):

1. 它从数字输入数据中获取每个项目来映射
2. 它使用pickle模块将项序列化为二进制数据(参见第68条)
3. 它通过本地套接字将序列化数据从主解释器进程复制到子解释器进程
4. 它使用子进程中的pickle将数据反序列化回Python对象
5. 它导入包含gcd函数的Python模块
6. 它与其他子进程并行地在输入数据上运行该函数
7. 它将结果序列化回二进制数据
8. 它通过套接字将二进制数据复制回去
9. 它将二进制数据反序列化回父进程中的Python对象
10. 它将多个子节点的结果合并到一个列表中以返回

​		虽然对程序员来说看起来很简单，但是多处理模块和ProcessPoolExecutor类做了大量的工作以使并行性成为可能。在大多数其他语言中，协调两个线程所需的唯一接触点是单个锁或原子操作(参见第54条)。通过ProcessPoolExecutor使用多处理的开销很高，因为所有的序列化和反序列化必须发生在父进程和子进程之间。

​		此方案非常适合于某些类型的隔离、高杠杆任务。所谓孤立，指的是那些不需要与程序的其他部分共享状态的函数。所谓高杠杆任务，指的是只需要在父进程和子进程之间传输少量数据就可以实现大量计算的情况。最伟大的公约数算法就是一个例子，但其他许多数学算法的工作原理类似。

​		如果您的计算没有这些特征，那么ProcessPoolExecutor的开销可能会阻止它通过并行化来加快程序的速度。当这种情况发生时，多处理为共享内存、跨进程锁、队列和代理提供了更高级的设施。但所有这些功能都非常复杂。很难在Python线程之间共享的单个进程的内存空间中推断这些工具。如果将这种复杂性扩展到其他进程并涉及到套接字，则会使其更加难以理解。

​		建议您一开始就避免使用多处理内置模块的所有部分。可以首先使用ThreadPoolExecutor类在线程中运行隔离的、高杠杆的函数。稍后可以移动到ProcessPoolExecutor以获得加速。最后，当完全使用了其他选项时，可以考虑直接使用multiprocessing模块。

**要点**

* 将CPU瓶颈转移到c扩展模块是一种提高性能的有效方法，同时最大限度地提高您在Python代码中的投资。但是，这样做的成本很高，可能会引入bug
* multiprocessing模块提供了强大的工具，可以以最少的努力并行化某些类型的Python计算
* multiprocessing模块的能力最好通过concurrent.future内置模块及其简单的ProcessPoolExecutor类实现
* 避免使用multiprocessing模块的高级(和复杂)部分，直到用尽了所有其他选项